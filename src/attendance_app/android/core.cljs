...
(def app-navigator
  (let [current-day (format-day "yyyy-MM-dd") day-to-show (format-day "E d 'of' MMMM")]
    (createStackNavigator (clj->js {:AttendantsList {:screen (r/reactify-component list-attendants)
                                                     :navigationOptions {:title day-to-show}}})
                          (clj->js
                           {:initialRouteName "AttendantsList"
                            :initialRouteParams {:day current-day}
